...
# ------------------------------------------------------------------------------
    ...
    def compile_tex(self):
        ...
        self.syc = self.tex.rsplit('.', 1)[0] + ".synctex"
...
# ------------------------------------------------------------------------------
    ...
    def write_output(self, box_separator="/", value_separator=" "):
        ...
        """
        f = open(self.out.rsplit('.', 1)[0] + ".txt", 'w')
            ...
            if line in self.records:
                for record in self.records[line]:
                    ...
                    f.write(str(record.page) + value_separator +
                            str(round(record.x, 3)) + value_separator +
                            str(round(record.y, 3)) + value_separator +
                            str(round(record.width, 3)) + value_separator +
                            str(round(record.height, 3)))
...
# ------------------------------------------------------------------------------
    ...
    def visualize(self, vis, idx):
        ...
        can = canvas.Canvas(os.path.dirname(os.path.abspath(__file__)) +
                            '/tmp/' + "tmp" + idx + ".pdf", pagesize=letter)
        ...
        inputStream2 = open(os.path.dirname(os.path.abspath(__file__)) +
                            '/tmp/' + "tmp" + idx + ".pdf", "rb")
        ...
        for root, dirs, files in os.walk(inp):
            for f in files:
                if f.endswith('.tex') and not f.endswith('pp.tex'):
                    ...
                    total += 1
                    name = out + "res_" + f.rsplit('.', 1)[0] + ".pdf"
                    if multipr:
