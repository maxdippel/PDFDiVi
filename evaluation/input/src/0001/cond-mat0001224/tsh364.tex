...
\be
  ...
  \left\{
    \begin{array}{lcl}
       \dot x_n & = & v_n, \\
        &&\\
       \dot v_n & = & A(1-\frac{v_nT+D}{x_{n+1}-x_n})\\
        &&\\
...
%We have simulated the model in presence of on- and off-ramps. c The results
%of local measurements of density and flux at different distances from the
%on-rump are shown in Figs. %3. We observe the gradual decrease of the local
%cars density and decrese %of the flux as the distance from the on-rump grows.
%That corresponds to %the gradual spatial transition from the fluctuative to
%free flow in the %downstream direction which was measured by \cite{KR1}.
%Figs.3 are in %good agreement with Fig.3 of \cite{KR1}.  
...
%: {\it Proceedings of Workshop on Traffic and Granular Flow (1995,
%Julich)}, edited by D.~E.~Wolf, M.~Schreckenberg, A.~Bachem (World
%Scientific, Singapore), 73 (1996).
